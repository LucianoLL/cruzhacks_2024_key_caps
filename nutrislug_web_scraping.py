...
url_add_on = "&WeeksMenus=UCSC+-+This+Week%27s+Menus&dtdate={}%2f{}%2f{}".format(date_url[0], date_url[1], date_url[2])
                ...
                if len(font_tags) == 2:
                    for m in MACROS:
                        if m in str(font_tags[0]):
                            amount = (re.search(regex_macros, str(font_tags[1])).group(0))[3:-8]
                            macro_data[m] = amount
...
async def fetch_and_process_menu_data(session, dh, meal, urls, url_add_on, headers, menu_json):
    menu_json[dh][meal] = []
    ...
    async with session.get(dh_meal_url, headers=headers[dh]) as long_menu:
        if long_menu.status == 200:
            ...
            all_day_tag = lm_soup.find('div', class_='longmenucolmenucat', string='-- All Day --') 
            if (all_day_tag):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for dh in headers.keys():
            menu_json[dh] = {}
            for meal in MEALS:
                tasks.append(fetch_and_process_menu_data(session, dh, meal, urls, url_add_on, headers, menu_json))
